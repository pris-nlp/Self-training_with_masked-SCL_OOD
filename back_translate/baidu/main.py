...
def trans(text: str,
    ...
    while True:
        try:
            ...
            output = res['trans_result']['data'][0]['dst']
            break
        except Exception:
            pass
    return output
...
if __name__ == "__main__":
    f = open('/home/disk1/zzy2019/PGN_pytorch_lightning-master/log/test.json')
    data = json.load(f)
    data = [i['dialogue'].split('\n') for i in data]
    fo = open('test_dialog.txt','w')
    for i in tqdm(range(len(data))):
        output = []
        for j in data[i]:
            tmp = trans(j,'en','zh-CN')
            print(tmp)
            output.append(tmp)
        fo.write('\n'.join(output)+'\n')
    # main()
